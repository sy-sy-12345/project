...
int speed=120, leftCount=0, rightCount=0, num=0;
int laterL=HIGH;
int laterR=HIGH;
unsigned long last=0, lastLCD=0;
const double distancePerHole=0.010210176;
double current_speed=0, distance=0, totalTime=0;
...
void loop(){
  ...
  else if(digitalRead(leftSensor)==LOW && digitalRead(rightSensor)==LOW) //stop
  {
    num++;
  }
  else 
...
int averageSpeed(double speedLeft, double speedRight)
{
  return current_speed = (speedRight+speedLeft)/2;
}
